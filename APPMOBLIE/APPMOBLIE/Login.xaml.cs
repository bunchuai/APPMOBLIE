...
namespace APPMOBLIE
{
    public partial class Login : ContentPage
    {
        ...
        private async void button_SignIn(object sender, EventArgs e)
        {
            ...
            if (string.IsNullOrEmpty(Email.Text) || Regex.IsMatch(Email.Text, emailRegex) == false)
            {
                displayError.Text = "อีเมลไม่ถูกต้อง";
            }
            else if (string.IsNullOrEmpty(Password.Text) || Email.Text.Length < 6)
            {
                displayError.Text = "รหัสผ่านไม่ถูกต้อง";
            }
            else
            {
